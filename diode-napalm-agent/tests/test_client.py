...
def test_init_client(mock_diode_client_class, mock_version):
    ...
    client = Client()
    client.init_client(target="https://example.com",
                       api_key="dummy_api_key", tls_verify=True)

    mock_diode_client_class.assert_called_once_with(
        target="https://example.com", app_name="diode-napalm-agent", app_version=mock_version(), api_key="dummy_api_key", tls_verify=True
    )
...
def test_ingest_success(mock_diode_client_class, sample_data):
    ...
    with patch('diode_napalm.client.translate_data', return_value=translate_data(sample_data)) as mock_translate_data:
